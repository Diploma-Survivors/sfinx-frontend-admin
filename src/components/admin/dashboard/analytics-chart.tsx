  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await DashboardService.getTimeSeriesMetrics(
          metric,
          fromStr,
          to,
        );
        if (response.data.data) {
          const dataMap: Record<
            DashboardMetric,
            keyof typeof response.data.data
          > = {
            users: "dailyNewUsers",
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="30d">
                  {t("chart.range.last30Days")}
                </SelectItem>
                <SelectItem value="month">
                  {t("chart.range.thisMonth")}
                </SelectItem>
                <SelectItem value="1y">{t("chart.range.last1Year")}</SelectItem>
                <SelectItem value="custom">
                  {t("chart.range.custom")}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="h-[350px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Area
                name={t(
                  `chart.metrics.${metric === "active_users" ? "activeUsers" : metric}`,
                )}
                type="monotone"
                dataKey="value"
                stroke="#8884d8"
