...
export function UserSubmissionStatsChart({
  stats,
}: UserSubmissionStatsChartProps) {
  const t = useTranslations("UserDetailPage.components.submissionStats");
      ...
      <CardContent className="flex items-center justify-between">
          ...
          >
            ...
            {(() => {
              ...
              const data = [
                ...
                {
                  status: "others",
                  count: stats.others + stats.compilationError,
                  color: "#6b7280",
                },
              ];
                  ...
                  {data.map((item) => {
                    ...
                    return (
                      <Tooltip
                        key={item.status}
                        content={`${
                          item.status === "others" ? t("others") : item.status
                        }: ${percent.toFixed(1)}%`}
                      >
                        <path
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{t("wrongAnswer")}:</span>
            <span className="font-bold">{stats.wrongAnswer}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-yellow-500" />
            <span className="text-muted-foreground">{t("timeLimit")}:</span>
            <span className="font-bold">{stats.timeLimitExceeded}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-orange-500" />
            <span className="text-muted-foreground">{t("runtimeError")}:</span>
            <span className="font-bold">{stats.runtimeError}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-gray-500" />
            <span className="text-muted-foreground">{t("others")}:</span>
            <span className="font-bold">
              {stats.others + stats.compilationError}
            </span>
          </div>
        </div>
      </CardContent>
