...
export function UserSolvedProblemsChart({
  stats,
}: UserSolvedProblemsChartProps) {
  const t = useTranslations("UserDetailPage.components.solvedProblems");
  ...
  return (
    <Card className="border border-border shadow-md bg-card">
      ...
      <CardContent className="flex items-center justify-center space-x-8">
        <Tooltip
          content={
            ((stats.total.solved / (stats.total.total || 1)) * 100).toFixed(1) +
            "%"
          }
        >
          <div className="relative w-32 h-32 flex items-center justify-center cursor-pointer">
            <svg viewBox="0 0 36 36" className="w-full h-full">
              ...
              <path
                ...
                strokeWidth="3"
                strokeDasharray={`${
                  (stats.total.solved / (stats.total.total || 1)) * 100
                }, 100`}
              />
            </svg>
        ...
        <div className="space-y-2 flex-1">
          ...
          <div className="w-full bg-muted rounded-full h-2">
            <div
              ...
              style={{
                width: `${
                  (stats.easy.solved / (stats.easy.total || 1)) * 100
                }%`,
              }}
            />
          </div>
          ...
          <div className="flex justify-between items-center text-sm">
            <span className="text-yellow-600 font-medium">{t("medium")}</span>
            <span className="font-bold">
              {stats.medium.solved}
              <span className="text-muted-foreground font-normal">
                /{stats.medium.total}
          ...
          <div className="w-full bg-muted rounded-full h-2">
            <div
              ...
              style={{
                width: `${
                  (stats.medium.solved / (stats.medium.total || 1)) * 100
                }%`,
              }}
            />
          </div>
          ...
          <div className="w-full bg-muted rounded-full h-2">
            <div
              ...
              style={{
                width: `${
                  (stats.hard.solved / (stats.hard.total || 1)) * 100
                }%`,
              }}
            />
          </div>
