...
export function FeatureEditDialog({ feature, open, onOpenChange, onSuccess }: FeatureEditDialogProps) {
    ...
    const fetchFeatureDetails = async (id: number) => {
        try {
            ...
            const enTranslation = featureData.translations?.find((t: any) => t.languageCode === 'en');
            const viTranslation = featureData.translations?.find((t: any) => t.languageCode === 'vi');
            ...
            toastService.error('Failed to load feature details');
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Edit Feature - {feature?.key}</DialogTitle>
                    <DialogDescription>
