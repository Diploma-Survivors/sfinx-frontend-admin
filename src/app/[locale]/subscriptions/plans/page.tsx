...
export default function PlansPage() {
  const t = useTranslations("Subscription.messages");
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await subscriptionService.getAllPlans(locale);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toastService.error(error.response?.data?.message || t("deleteError"));
    } finally {
      setIsDeleting(false);
    }
  };
